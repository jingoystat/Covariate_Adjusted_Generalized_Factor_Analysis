...
anchor_num <- as.numeric(args[[8]])
anchor_type <- as.numeric(args[[9]]) # 1 - dif 2 - non-dif
...
for (rep in 1:100){
  ...
  am_est <- AM_estimation_anchor(Y, X, r, tol=0.0001, tau=3, ite=3, maxiter=20, anchor_num=anchor_num, anchor_type=anchor_type, seed=rep_seed) ## anchor_type = 1 (dif); 2(non-dif)
  ...
  ## set anchor covariate effect
  if (anchor_type == 1){
    Beta_final[1:anchor_num, 2:(p+1)] <- 0
  }
  
  if (anchor_type == 2){
    Beta_final[(q-(anchor_num)+1):q, 2:(p+1)] <- 0
  }
  
  Beta_hat <- Beta_final # cbind(Beta_final[,1], Beta_hat)
...
write.table(Beta_ind_pvalue_full,sep=",",  col.names=FALSE, row.names=FALSE, file=
              paste0('Beta7_ind_pvalue_full_n',n,'_p_',p,'_q_',q,'_r_',r,'_signal_',signal,'_nrep_',nrep,'_rho_',rho,'_anchor_num_',anchor_num,'_anchor_type_',anchor_type,'.csv'))

write.table(Beta_est,sep=",",  col.names=FALSE, row.names=FALSE, file=
              paste0('Beta_est7_n',n,'_p_',p,'_q_',q,'_r_',r,'_signal_',signal,'_nrep_',nrep,'_rho_',rho,'_anchor_num_',anchor_num,'_anchor_type_',anchor_type,'.csv'))

write.table(Gamma_est,sep=",",  col.names=FALSE, row.names=FALSE, file=
              paste0('Gamma_est7_n',n,'_p_',p,'_q_',q,'_r_',r,'_signal_',signal,'_nrep_',nrep,'_rho_',rho,'_anchor_num_',anchor_num,'_anchor_type_',anchor_type,'.csv'))

write.table(U_est,sep=",",  col.names=FALSE, row.names=FALSE, file=
              paste0('U_est7_n',n,'_p_',p,'_q_',q,'_r_',r,'_signal_',signal,'_nrep_',nrep,'_rho_',rho,'_anchor_num_',anchor_num,'_anchor_type_',anchor_type,'.csv'))
