...
for (rep in 1:100){
  ...
  for(i in 1:n){
    ...
    #U_stat <- c(U_stat, sqrt(q)* sqrtm(solve(sigma_ui)) %*%(U_final[i,] - U_true[i,]))
  ...
  #A_hat <- t(solve(t(X) %*% X) %*% t(X) %*% U_final)
  
  ## The following few steps try to convert back the mle before transformation 
  ## in order to get the estimator for covariance matrix
  ...
  for(j in 1:q){
    ...
    #Gamma_stat <- c(Gamma_stat, sqrt(n)*sqrtm(solve(sigma_gj)) %*%(Gamma_final[j,] - Gamma_true[j,]))
    ...
    #Beta_stat <- rbind(Beta_stat, sqrt(n)*sqrtm(solve(sigma_beta)) %*% (Beta_final[j,2:(p+1)]-Beta_true[j,2:(p+1)]))
    ...
    #chisq_stat <- t(Beta_final[j,]) %*% solve(sigma_bj) %*% Beta_final[j,]
    #pvalue_chisq <- 1 - pchisq(chisq_stat, df=p)
...
write.table(Beta_ind_pvalue_full,sep=",",  col.names=FALSE, row.names=FALSE, file=
              paste0('Beta1_ind_pvalue_full_n',n,'_p_',p,'_q_',q,'_r_',r,'_signal_',signal,'_nrep_',nrep,'_rho_',rho,'.csv'))

write.table(Beta_est,sep=",",  col.names=FALSE, row.names=FALSE, file=
              paste0('Beta_est1_n',n,'_p_',p,'_q_',q,'_r_',r,'_signal_',signal,'_nrep_',nrep,'_rho_',rho,'.csv'))

write.table(Gamma_est,sep=",",  col.names=FALSE, row.names=FALSE, file=
              paste0('Gamma_est1_n',n,'_p_',p,'_q_',q,'_r_',r,'_signal_',signal,'_nrep_',nrep,'_rho_',rho,'.csv'))

write.table(U_est,sep=",",  col.names=FALSE, row.names=FALSE, file=
              paste0('U_est1_n',n,'_p_',p,'_q_',q,'_r_',r,'_signal_',signal,'_nrep_',nrep,'_rho_',rho,'.csv'))
