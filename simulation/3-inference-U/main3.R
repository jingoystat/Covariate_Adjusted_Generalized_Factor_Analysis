...
size_ind_U_full <- NULL
power_ind_U_full <- NULL

cover_U_full <- NULL
cover_mtx_U_full <- NULL
...
for (rep in 1:100){
  ...
  am_est <- AM_estimation(Y, X, r, tol=0.0001, tau=3, ite=3, maxiter=20, seed=rep_seed)
  ...
  ## address the sign flip issue
  perms <- permutations(n = r, r = r, v = 1:r)
  diff <- c()
  for(i in 1:nrow(perms)){
    diff[i] <- 0
    for(d in 1:r)
    {
      dif1 <- sum((Gamma_final[, perms[i, d]] - Gamma_true[, d])^2)
      dif2 <- sum((Gamma_final[, perms[i, d]] + Gamma_true[, d])^2)
      diff[i] <- diff[i] + min(dif1, dif2)
    }
  }
  
  min_per <- which.min(diff)
  t_factors <- matrix(0, n, r)
  t_loadings <- matrix(0, q, r)
  for(d in 1:r)
  {
    if(sum((Gamma_final[, perms[min_per, d]] - Gamma_true[, d])^2) > sum((Gamma_final[, perms[min_per, d]] + Gamma_true[, d])^2))
    {
      t_loadings[, d] <- - Gamma_final[, perms[min_per, d]]
      t_factors[, d] <- -U_final[, perms[min_per, d]]
    }
    else{
      t_loadings[, d] <- Gamma_final[, perms[min_per, d]]
      t_factors[, d] <- U_final[, perms[min_per, d]]
  ...
  U_ind_pvalue <- NULL
  covered_U <- matrix(FALSE, nrow = n, ncol = r)
  ...
  for(i in 1:n){
    ...
    for(k in 1:r){
      ci_U_upper <- U_final[i,k] + qnorm(0.975)*sqrt(diag(sigma_ui)[k])/sqrt(q)
      ci_U_lower <- U_final[i,k] - qnorm(0.975)*sqrt(diag(sigma_ui)[k])/sqrt(q)
      
      covered_U[i, k] <- (U_true[i, k] >= ci_U_lower) & (U_true[i, k] <= ci_U_upper)
    }
  ...
  cover_U_full <- c(cover_U_full, mean(covered_U))
  cover_mtx_U_full <- rbind(cover_mtx_U_full, covered_U)
  ...
  U_ind_pvalue_full <- rbind(U_ind_pvalue_full, U_ind_pvalue)
  
  
}

write.table(cover_U_full,sep=",",  col.names=FALSE, row.names=FALSE, file=
              paste0('cover9_U_full_n',n,'_p_',p,'_q_',q,'_r_',r,'_signal_',signal,'_nrep_',nrep,'_rho_',rho,'.csv'))

write.table(cover_mtx_U_full,sep=",",  col.names=FALSE, row.names=FALSE, file=
              paste0('cover_mtx9_U_full_full_n',n,'_p_',p,'_q_',q,'_r_',r,'_signal_',signal,'_nrep_',nrep,'_rho_',rho,'.csv'))

write.table(Beta_ind_pvalue_full,sep=",",  col.names=FALSE, row.names=FALSE, file=
              paste0('Beta9_ind_pvalue_full_n',n,'_p_',p,'_q_',q,'_r_',r,'_signal_',signal,'_nrep_',nrep,'_rho_',rho,'.csv'))

write.table(Beta_est,sep=",",  col.names=FALSE, row.names=FALSE, file=
              paste0('Beta_est9_n',n,'_p_',p,'_q_',q,'_r_',r,'_signal_',signal,'_nrep_',nrep,'_rho_',rho,'.csv'))

write.table(Gamma_est,sep=",",  col.names=FALSE, row.names=FALSE, file=
              paste0('Gamma_est9_n',n,'_p_',p,'_q_',q,'_r_',r,'_signal_',signal,'_nrep_',nrep,'_rho_',rho,'.csv'))

write.table(U_est,sep=",",  col.names=FALSE, row.names=FALSE, file=
              paste0('U_est9_n',n,'_p_',p,'_q_',q,'_r_',r,'_signal_',signal,'_nrep_',nrep,'_rho_',rho,'.csv'))
