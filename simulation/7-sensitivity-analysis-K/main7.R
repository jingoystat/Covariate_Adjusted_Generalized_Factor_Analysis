...
for (rep in 1:100){
  ...
  am_est <- AM_estimation(Y, X, r_mis, tol=0.0001, tau=3, ite=3, maxiter=20, seed=rep_seed)
  ...
  norm0 <- normalization(Y, X, r_mis, U_hat, Gamma_final)
  U1 <- norm0$U1
  Gamma1 <- norm0$Gamma1
  A_hat <- -A_hat
  ...
  for(j in 1:q){
    ...
    sigma_gamma_beta_j1 <- matrix(0, nrow=r_mis, ncol=p+1)
...
write.table(Beta_ind_pvalue_full,sep=",",  col.names=FALSE, row.names=FALSE, file=
              paste0('Beta6_ind_pvalue_full_n',n,'_p_',p,'_q_',q,'_r_',r,'_signal_',signal,'_nrep_',nrep,'_rho_',rho,'_r_mis_',r_mis,'.csv'))

write.table(Beta_est,sep=",",  col.names=FALSE, row.names=FALSE, file=
              paste0('Beta_est6_n',n,'_p_',p,'_q_',q,'_r_',r,'_signal_',signal,'_nrep_',nrep,'_rho_',rho,'_r_mis_',r_mis,'.csv'))

write.table(Gamma_est,sep=",",  col.names=FALSE, row.names=FALSE, file=
              paste0('Gamma_est6_n',n,'_p_',p,'_q_',q,'_r_',r,'_signal_',signal,'_nrep_',nrep,'_rho_',rho,'_r_mis_',r_mis,'.csv'))

write.table(U_est,sep=",",  col.names=FALSE, row.names=FALSE, file=
              paste0('U_est6_n',n,'_p_',p,'_q_',q,'_r_',r,'_signal_',signal,'_nrep_',nrep,'_rho_',rho,'_r_mis_',r_mis,'.csv'))
