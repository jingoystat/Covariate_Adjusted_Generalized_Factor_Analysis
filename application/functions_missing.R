...
generate_parameter = function(n, p, q, r, signal, seed)
{
  ...
  Gamma <- NULL
  for(k in 1:r){
    subgamma[,k] <- runif(subg, 0.5, 1.5) #rep(0.5,subg) #  ; 
  ...
  U <- matrix(rnorm(n*r, 0, 1), nrow=n, ncol=r)
  for(k in 1:r){
    U[,k] = U[,k] - mean(U[,k])
    for(i in 1:n)
      if(abs(U[i,k])>2)
  ...
  Beta <- rbind(matrix(0, r, p), subbeta, matrix(0, q-r-nrow(subbeta), p)) # Fix r items in B to be all zeros
  
  Beta_intercept <-  c(rep(0,q)) #c(rep(0, r), rnorm(q-r, 0, 2))  #   
  Beta <- cbind(Beta_intercept, Beta)
  
  
  return(list("U"=U_true, "Gamma"=Gamma_true, "Beta"=Beta))
...
AM_estimation=function(Y, X, r, tol=0.001, tau=5, ite=1, maxiter=20, seed)
{
  ...
  for (t in 1:ite){
    ...
    for (j in 1:q){
        ...
        fit_j <- glm(Y[index_Nj,j] ~ U0[index_Nj,] + X[index_Nj,] -1,  family=binomial()) 
        coef_j <- coef(fit_j)
    ...
    for (i in 1:n){
      ...
      fit_i <- glm(Y[i, index_Ji] ~ Gamma1[index_Ji,] + offset(Beta1[index_Ji,] %*% X[i, ])-1, family=binomial()) #, intercept=FALSE)
      coef_i <- coef(fit_i)
      # truncate the U1
...
AM_estimation_bcmk=function(Y, X, r, tol=0.001, tau=5, ite=1, maxiter=20, seed)
{
  ...
  for (t in 1:ite){
    ...
    Gamma1 <- matrix(0, nrow=q, ncol=r)  
    U1 <- matrix(0, nrow=n, ncol=r)
    ...
    # iteration
    # first try glmnet, if it does not work, try (1) optim (2) ao
    # try initial values by PCA
    iter <- 0 
    while (norm(Beta1-Beta0, "2")/q > tol){
      #for (s in 1:1){
      #Gamma0 <- Gamma1
      Beta0 <- Beta1
      #U0 <- U1
