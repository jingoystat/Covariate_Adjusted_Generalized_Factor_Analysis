...
generate_para_data = function(n, p, q, r, rho=0.2, signal, seed)
{
  ...
  set.seed(seed)
  #Gamma <- matrix(runif(q*r, 1.5, 2.5), nrow=q, ncol=r)
  subg <- q/r
...
### Response generating function  - true logit
...
AM_estimation=function(Y, X, r, tol=0.001, tau=5, ite=1, maxiter=20, seed)
{
  ...
  for (t in 1:ite){
    ...
    #mirt_init <- mirt(data=as.data.frame(Y), r, itemtype = "2PL", verbose = T, technical = list(NCYCLES=100))
    #Gamma0 <- extract.mirt(mirt_init, "F")
    #U0 <- fscores(mirt_init)
    
    #mirtjml_expr(response=as.data.frame(Y), K=r)
    ...
    for (i in 1:n){
      fit_i <- glm(Y[i, ] ~ Gamma0 + offset(Beta0 %*% X[i, ])-1, family=gaussian) #, intercept=FALSE)
      coef_i <- coef(fit_i)
      # truncate the U1
    ...
    for (j in 1:q){
      fit_j <- glm(Y[,j] ~ U1 + X -1, family=gaussian)
      coef_j <- coef(fit_j)
